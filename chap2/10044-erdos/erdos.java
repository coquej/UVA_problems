...
class Autor
{
	...
	public void conectar_con( Autor coautor ){this.coautores.add( coautor );}

	public String getNombre() { return nombre; }
	public int getErdosNumber() { return erdos_number; }
	public void setErdosNumber( int n ) { erdos_number = n; }

	public LinkedList<Autor> getCoautores(){return coautores;}
...
class Diccionario
{
	...
	public Diccionario(){ raiz = new Nodo(); }
	...
	public void recorre(){ raiz.recorre( "" );}}
...
class Main
{
	...
	public static void main( String args[] )
	{
		...
		while( ++escenario <= escenarios ) {
			...
			// A Erdos lo añadimos a piñon fijo. Porque nos pueden poner un caso
			// trampa en el que no esté Erdoss y la búsqueda falle estrepitosamente.
			...
			// Calcular los numeros de Erdos mediante un arbol de expansion mi­nima

			Autor a2;
			int num=0;
			LinkedList<Autor> visitados = new LinkedList<Autor>();

			Autor a1= dicc.get("Erdos, P.");
			visitados.add(a1);
			a1.setErdosNumber(0);

			LinkedList<Autor> coautores = a1.getCoautores();
			num=a1.getErdosNumber()+1;
			int visitados_total=1;
			for(int i=0;i<visitados_total;i++){
				a1=visitados.get(i);
				coautores = a1.getCoautores();
				num=a1.getErdosNumber()+1;
				for(int j=0;j<coautores.size();j++){
					a2=coautores.get(j);
					if(visitados.indexOf(a2)==-1){
						visitados.add(a2);
						visitados_total++;
						a2.setErdosNumber(num);
					}
				}
			}
